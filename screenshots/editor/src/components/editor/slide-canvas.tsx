...
import { RenderAccent, buildBackground } from "./accents";
      ...
      <div
        style={{
          ...
          boxShadow: inverted
            ...
            : "0 8px 22px -10px rgba(20,8,2,0.25), inset 0 0 0 1px rgba(20,8,2,0.06)",
      ...
      >
        <span
          style={{
            ...
            boxShadow: `0 0 ${unit * 0.018}px ${accent}`,
          }}
        />
        <EditableText
...
// ---------- Background ----------
// Default backgrounds now flow through buildBackground (accents.tsx) which
// supports more styles. This local helper stays for legacy fallback.
...
export function SlideCanvas({
  ...
  const bg = buildBackground(slide.bgStyle ?? "soft", theme, inverted);
  ...
  return (
    ...
    >
      <Blob cW={cW} color={theme.accent} x={-15} y={-10} size={55} opacity={inverted ? 0.18 : 0.22} />
      <Blob cW={cW} color={theme.accent} x={70} y={75} size={45} opacity={inverted ? 0.12 : 0.18} />

      {(slide.accents || []).map((a, i) => (
        <RenderAccent key={i} cW={cW} cH={cH} theme={theme} inverted={inverted} accent={a} />
      ))}
