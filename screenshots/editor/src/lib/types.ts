...
// Decorative overlays drawn behind the caption + device. Positions are
// percentages of the canvas (0-100). `rotate` is degrees, `size` is a width
// percentage of the canvas. Each kind has its own visual.
export type SlideAccent =
  | {
      ...
      rotate?: number;
      tone?: "accent" | "light" | "dark" | "ghost";
      icon?: "spark" | "heart" | "check" | "fire" | "leaf" | "bell" | "voice" | "camera" | "barcode" | "health";
      arrowDir?: "left" | "right" | "down" | "up" | "down-left" | "down-right";
    }
  | {
      kind: "stat";
      value: string;
  ...
  | {
      kind: "badge";
      label: string; // small caps eyebrow inside badge
  ...
  | {
      ...
      color?: string;
      opacity?: number;
      width?: number; // stroke width in % of canvas
    };
...
export type Slide = {
  ...
  // Optional decorations layered behind the caption + device.
  accents?: SlideAccent[];
  // Background variant — overrides the theme's default linear gradient.
  bgStyle?: "gradient" | "block" | "dots" | "mesh" | "soft";
};
