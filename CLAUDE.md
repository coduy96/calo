...
- **App Review rules learned the hard way** (re-check before every submission):
  ...
  - **HealthKit pre-prompt copy + flow** (Guideline 5.1.1(iv)) — the v3.2 (5) onboarding "Connect to Apple Health" screen got rejected for two things: button text "Connect" (use "Continue" or "Next" instead since the OS dialog is where consent legally happens) AND the "Skip" button (Apple wants users to ALWAYS proceed to the OS permission dialog after the message — the OS dialog itself is where the Allow/Don't Allow choice lives). Fix: rename the button to "Continue" and remove any Skip option from pre-prompt screens. Same rule applies to any future permission pre-prompt (camera, microphone, notifications, location).
  - **Medical / health calculations need citations** (Guideline 1.4.1) — the v3.2 (5) "Your Plan" onboarding screen got rejected for showing calorie + macro numbers without citing the source formulas. Fix in v3.2 (6): added `Views/CalculationMethodsView.swift` (Mifflin-St Jeor + Katch-McArdle BMR, FAO/WHO/UNU activity multipliers, Hall 2011 energy balance, Morton 2018 protein meta-analysis, USDA FoodData Central for micros — each with peer-reviewed citation + clickable Link to PubMed/Lancet/FAO/BJSM). Reachable from the Plan onboarding step ("How is this calculated?" button) AND Settings → Goals & Nutrition → Calculation Methods. Citation must be "easy for the user to find" per Apple — both entry points are mandatory.
  - **Third-party AI service requires explicit in-app consent** (Guideline 5.1.1(i) + 5.1.2(i)) — the v3.2 (5) submission got rejected for sending food photos / voice transcripts / text descriptions / Coach profile data to third-party AI providers (Gemini, OpenAI, etc.) without an explicit in-app consent prompt. Apple is explicit: "only including this information in the app's Terms of Service or Privacy Policy is not sufficient." Fix in v3.2 (6): added `AIConsentSheetView` in ContentView.swift gated by `@AppStorage("aiAnalysisConsentGiven")`. Sheet must (a) disclose what data is sent (photo / voice / text / profile data for Coach), (b) name the actual provider via dynamic %@ format from `AIProviderSettings.selectedProvider.rawValue`, (c) have explicit Allow / Not Now buttons + `.interactiveDismissDisabled()` so users must tap one of the two. Gates all 5 AI entry points (text input, voice, photo picker, camera capture, nutrition label). Manual entry stays unblocked since it doesn't hit any AI provider. If the privacy policy is updated to add new AI features in the future, also re-verify the in-app consent sheet still describes them.
  - **No external donation links** (Guideline 3.1.1 — In-App Purchase) — the v3.2 (6) submission got rejected for the "Support the Project" link in the About section pointing to paypal.me/apoorvdarshan. Apple treats donations to app developers as digital content/services that must use IAP (Apple takes 30%). Fix in v3.2 (7): removed the entire `Link(destination: URL(string: "https://paypal.me/apoorvdarshan")!) { Label { Text("Support the Project") ... } }` block from ContentView.swift About section (line ~128). Donations remain available via the marketing site (https://fud-ai.app), GitHub README, and X bio — Apple has no jurisdiction over those. Don't reintroduce a PayPal/Stripe/Patreon/Ko-fi/Buy Me A Coffee link in the iOS app under any framing ("Tip Jar", "Buy me a coffee", "Support development" all trigger the same rule). If donations EVER need to be in the app, the only Apple-compliant path is StoreKit IAP with Apple taking 30% — which contradicts the free-forever brand and is not worth the friction. Same rule applies to any future "premium tier" or "thank-you" mechanism.
- **`PLAYSTORE.md`** holds Play Console listing copy for Android — App Name (English source only), Short Description (English source only, must avoid price/promotion keywords like "free"/"discount" or Play Console blocks listing promotion), Full Description (English source only, ~4000 char cap), What's New (15-lang `<lang-tag>` block, paste-ready into the release notes field), categorization, contact info, App content declarations. App Name + Short Description + Full Description stay English-only on Play Console — non-English browsers see the English source as fallback (deliberate; the in-app UI itself is fully translated via per-locale `values-{lang}/strings.xml` so the localization gap is only on the listing surface). What's New IS translated into all 14 non-English locales since the per-language `<lang-tag>` blocks are pasted directly into the Release notes field. **Full Description has a 4000-char hard cap** (Play Console truncates anything over). Update on every Android version bump (current header: `v1.0.6 / versionCode 7`). APPSTORE.md ↔ PLAYSTORE.md are intentionally separate since Play has no Apple-Guideline-2.3.10 restriction (Play description CAN mention iOS / cross-platform availability if useful — currently doesn't, both store listings stay self-contained)
